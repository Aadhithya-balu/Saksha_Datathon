        ...
        <div className="lg:col-span-7 bg-[#111D35]/30 border border-border-color p-5 rounded-card flex flex-col justify-between overflow-hidden">
          <div className="flex flex-col gap-4 overflow-hidden flex-1">
            <div className="flex justify-between items-center select-none border-b border-slate-900 pb-2">
              <span className="text-[10px] font-mono font-bold text-[#E8EDF5] uppercase tracking-wider">Offender Dossier Database</span>
              ...
              <div className="md:col-span-4 overflow-y-auto pr-1 flex flex-col gap-2 custom-scrollbar max-h-[300px]">
                {filteredOffenders.length ? filteredOffenders.map((item) => (
                  ...
                  >
                    ...
                    <span className="text-[8px] text-[#6A7A96] block mt-0.5">{item.name}</span>
                  </button>
                )) : (
                  <div className="p-4 text-center text-[9px] font-mono text-[#6A7A96] uppercase border border-dashed border-slate-800 rounded">
                  ...
                  <div className="space-y-4">
                    <div className="p-3 bg-slate-950/50 border border-slate-950 rounded flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-[13px] font-extrabold text-white mt-1.5 truncate">{activeOffender.name}</h4>
                        <span className="text-[9.5px] text-[#A8B4CC] block mt-0.5 uppercase tracking-wide">ALIAS: {activeOffender.alias}</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      ...
                      <div className="p-2 border border-slate-900 rounded">
                        <span className="text-slate-500 uppercase text-[8px] block">Primary pattern</span>
                      ...
                      <div className="p-2 border border-slate-900 rounded">
                        ...
                        <span className="text-white font-bold block mt-0.5 truncate">{activeOffender.activeDistricts.join(', ') || 'No linked districts'}</span>
                      </div>
                    </div>

                    <div className="p-3 border border-slate-900 rounded text-[#A8B4CC] leading-relaxed">
                      {activeOffender.mugshotDesc}
                    </div>
                  </div>
                ) : (
                  <div className="p-6 text-center text-[#6A7A96] uppercase">No backend profile highlighted</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3 overflow-hidden flex-1 select-none">
            <div className="flex justify-between items-center border-b border-slate-900 pb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono font-bold text-[#E8EDF5] uppercase tracking-wider">Cryptographic System Audits</span>
              </div>
              <button onClick={clearLogs} className="text-[8px] font-mono bg-slate-900 hover:bg-slate-800 border border-slate-800 px-2 py-0.5 rounded text-amber-500 cursor-pointer">
                Clear Screen
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-2 custom-scrollbar max-h-[360px]">
              {logs.map((log) => {
                const color = log.actionType === 'EXPORT' ? 'text-red-400' : log.actionType === 'AUTH' ? 'text-emerald-400' : log.actionType === 'REVIEW' ? 'text-sky-400' : log.actionType === 'ESCALATION' ? 'text-purple-400' : 'text-[#A8B4CC]';
                return (
