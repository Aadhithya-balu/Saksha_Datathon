...
export const Offenders: React.FC = () => {
  ...
  const [selectedWatermark, setSelectedWatermark] = useState('CONFIDENTIAL - SCRB BEATS');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-[var(--border-muted)] pb-3">
        <div>
          ...
          <p className="text-[9.5px] font-mono text-[var(--text-muted)] mt-0.5">
            LAW ENFORCEMENT BIO-REGISTRY - BACKEND DOSSIERS & CLASSIFIED WATERMARK EXPORTS
          </p>
          {loadError && <p className="text-[9px] font-mono text-amber-400 uppercase mt-1">{loadError}</p>}
        </div>
      </div>
        ...
        <div className="lg:col-span-7 bg-[var(--bg-tertiary)]/30 border border-border-color p-5 rounded-card flex flex-col justify-between overflow-hidden">
          <div className="flex flex-col gap-4 overflow-hidden flex-1">
            <div className="flex justify-between items-center select-none border-b border-[var(--border-primary)] pb-2">
              <span className="text-[10px] font-mono font-bold text-[var(--text-primary)] uppercase tracking-wider">Offender Dossier Database</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 flex-grow overflow-hidden">
              <div className="md:col-span-4 overflow-y-auto pr-1 flex flex-col gap-2 custom-scrollbar max-h-[300px]">
                {filteredOffenders.length ? filteredOffenders.map((item) => (
                  <button
                    key={item.id}
                  ...
                  >
                    ...
                    <span className="text-[8px] text-[var(--text-muted)] block mt-0.5">{item.name}</span>
                  </button>
                )) : (
              ...
              <div className="md:col-span-8 overflow-y-auto pr-1 flex flex-col gap-3 custom-scrollbar text-xs font-mono max-h-[300px]">
                {activeOffender ? (
                  <div className="space-y-4">
                    <div className="p-3 bg-[var(--bg-secondary)]/50 border border-[var(--border-primary)] rounded flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <h4 className="text-[13px] font-extrabold text-[var(--text-primary)] mt-1.5 truncate">{activeOffender.name}</h4>
                        <span className="text-[9.5px] text-[var(--text-secondary)] block mt-0.5 uppercase tracking-wide">ALIAS: {activeOffender.alias}</span>
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2 text-[10px]">
                      <div className="p-2 border border-[var(--border-primary)] rounded">
                        <span className="text-[var(--text-muted)] uppercase text-[8px] block">Threat index</span>
                        <span className="text-[var(--text-primary)] font-bold block mt-0.5">{activeOffender.riskScore}% severity</span>
                      </div>
                      <div className="p-2 border border-[var(--border-primary)] rounded">
                        <span className="text-[var(--text-muted)] uppercase text-[8px] block">Operational state</span>
                        <span className="text-emerald-400 font-bold block mt-0.5 uppercase">{activeOffender.status}</span>
                      </div>
                      <div className="p-2 border border-[var(--border-primary)] rounded">
                        <span className="text-[var(--text-muted)] uppercase text-[8px] block">Primary pattern</span>
                        <span className="text-[var(--text-primary)] font-bold block mt-0.5 truncate">{activeOffender.gangAffiliation}</span>
                      </div>
                      <div className="p-2 border border-[var(--border-primary)] rounded">
                        <span className="text-[var(--text-muted)] uppercase text-[8px] block">Key sectors</span>
                        <span className="text-[var(--text-primary)] font-bold block mt-0.5 truncate">{activeOffender.activeDistricts.join(', ') || 'No linked districts'}</span>
                      </div>
                    </div>
                  ...
                  <div className="p-6 text-center text-[var(--text-muted)] uppercase">No backend profile highlighted</div>
                )}
              </div>
            </div>
          </div>
          ...
          {activeOffender && (
            <div className="pt-4 border-t border-border-color mt-4 flex flex-col sm:flex-row gap-3 items-end">
              <div className="flex-1 w-full flex flex-col gap-1 text-[10px] font-mono text-left">
                <span className="text-[var(--text-muted)] uppercase font-bold">Select Security Watermark String</span>
                <select
              ...
              <button
                ...
                className="w-full sm:w-auto py-2 px-5 bg-[#C94A2A] hover:bg-[#C94A2A]/85 text-[var(--text-primary)] font-mono text-[10px] uppercase rounded-btn font-semibold tracking-wider flex items-center justify-center gap-1.5 shadow-glow-coral cursor-pointer"
              >
          ...
          <div className="flex flex-col gap-3 overflow-hidden flex-1 select-none">
            <div className="flex justify-between items-center border-b border-[var(--border-primary)] pb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono font-bold text-[var(--text-primary)] uppercase tracking-wider">Cryptographic System Audits</span>
              </div>
              <button onClick={clearLogs} className="text-[8px] font-mono bg-[var(--bg-tertiary)] hover:bg-[var(--bg-elevated)] border border-[var(--border-primary)] px-2 py-0.5 rounded text-amber-500 cursor-pointer">
                Clear Screen
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 flex flex-col gap-2 custom-scrollbar max-h-[360px]">
              {logs.map((log) => {
                const color = log.actionType === 'EXPORT' ? 'text-red-400' : log.actionType === 'AUTH' ? 'text-emerald-400' : log.actionType === 'REVIEW' ? 'text-sky-400' : log.actionType === 'ESCALATION' ? 'text-purple-400' : 'text-[var(--text-secondary)]';
                return (
                  <div key={log.id} className="text-[8.5px] font-mono leading-relaxed border-b border-[var(--border-muted)] pb-1 flex flex-col gap-0.5 text-left">
                    ...
                    <div className={color}>
                      <span className="text-[var(--text-primary)] font-semibold">[{log.badgeId}]</span> {log.details}
                    </div>
                  </div>
                );
              })}
