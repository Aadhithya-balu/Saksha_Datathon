  ...
  return (
    <div className="space-y-6 max-w-3xl mx-auto font-mono text-xs">
      ...
      <div className="flex justify-between items-center pb-4 border-b border-border-color">
        ...
        <h2 className="text-sm font-bold text-white uppercase tracking-wider">Modify Case Record [{caseNumber}]</h2>
      </div>
      ...
      {/* Main Form */}
      <form onSubmit={handleSubmit} className="p-6 bg-secondary-bg border border-border-color rounded-card space-y-5 text-[#A8B4CC]">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {/* Status Select */}
          <div className="flex flex-col gap-1.5">
            <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Clearance Workflow Status</label>
            <select
              value={status}
          ...
          {/* Priority Select */}
          <div className="flex flex-col gap-1.5">
            <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Threat Priority Level</label>
            <select
              value={priority}
          ...
          {/* Progress Percent */}
          <div className="flex flex-col gap-1.5">
            <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Progress Percentage ({progress}%)</label>
            <input
              type="range"
              min="0"
              max="100"
              ...
              className="w-full h-1 bg-slate-900 rounded-lg appearance-none cursor-pointer border border-[#1E6FD9]/20"
            />
          </div>

          {/* Assigned Officer */}
          <div className="flex flex-col gap-1.5">
            <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Assigned Investigating Officer</label>
            <select
        ...
        {/* MO Tags */}
        <div className="flex flex-col gap-1.5">
          <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Modus Operandi Tags (Comma-Separated)</label>
          <input
            type="text"
        ...
        <div className="flex flex-col gap-1.5">
          <label className="uppercase text-[10px] text-[#6A7A96] font-bold">Narrative Description Brief</label>
          <textarea
            rows={5}
            value={description}
        ...
        <div className="flex justify-end gap-3 pt-3 border-t border-border-color/30">
          ...
          <button
            type="submit"
            disabled={saving}
            className="flex items-center gap-2 px-5 py-2 bg-[#1E6FD9] hover:bg-[#1E6FD9]/80 disabled:opacity-50 transition-colors rounded uppercase font-bold text-white cursor-pointer"
          >
