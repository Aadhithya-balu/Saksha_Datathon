  ...
  if (criminals.length === 0) {
    return (
      <div className="p-5 bg-secondary-bg border border-border-color rounded-card">
        ...
        <p className="text-[10px] text-[#6A7A96] py-4 text-center uppercase">No criminals linked to this case.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-5 bg-secondary-bg border border-border-color rounded-card">
      <div className="flex justify-between items-center mb-4 border-b border-border-color/60 pb-3">
        ...
        <span className="text-[10px] text-[#6A7A96] font-bold">{criminals.length} NAMED</span>
      </div>

      <div className="space-y-3 max-h-[350px] overflow-y-auto custom-scrollbar pr-1">
        {criminals.map((criminal) => (
          <div key={criminal.id} className="p-3 bg-slate-950/40 border border-border-color/40 rounded hover:border-red-900/30 transition-colors">
            {/* Header */}
            <div className="flex justify-between items-start">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold text-white uppercase">{criminal.full_name}</span>
                  <span className={`text-[8px] uppercase font-bold ${getStatusColor(criminal.status)}`}>
                    [{criminal.status.replace(/_/g, ' ')}]
                  </span>
                </div>
                {criminal.aliases && (
                  <span className="text-[8px] text-[#6A7A96] mt-0.5 block">ALIAS: {criminal.aliases}</span>
                )}
              </div>
            ...
            {/* Details */}
            <div className="grid grid-cols-2 gap-2 mt-2 text-[8.5px] text-[#A8B4CC]">
              {criminal.gender && (
                <span>GENDER: {criminal.gender}</span>
              )}
              <span className="flex items-center gap-1">
                <FileText className="w-3 h-3 text-[#6A7A96]" />
            ...
            {criminal.identifying_marks && (
              <div className="flex items-start gap-1.5 mt-2 text-[8px] text-[#6A7A96]">
            ...
            {/* MO Summary */}
            {criminal.mo_summary && (
              <div className="flex items-start gap-1.5 mt-1.5 text-[8px] text-[#6A7A96]">
