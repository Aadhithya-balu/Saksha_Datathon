...
// ── Graph-Based Network Intelligence APIs ──
...
export interface ShortestPathResult {
  found: boolean;
  distance: number;
  path_nodes: Array<{ id: string; name: string; category: string; riskScore: number; details: string; casesCount: number }>;
  path_edges: Array<{ source: string; target: string; relationship: string }>;
  explanation: string;
}
...
export async function getFullNetworkGraph(categoryFilter?: string, minRisk?: number) {
  return apiRequest<{ nodes: any[]; edges: any[]; total_nodes: number; total_edges: number; is_neo4j_backed: boolean }>(
    `/network/graph${buildQueryString({ category_filter: categoryFilter, min_risk: minRisk })}`
  );
}
