...
export const CriminalGraph3D: React.FC<CriminalGraph3DProps> = ({ onNodeSelect }) => {
  const fgRef = useRef<any>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [graphData, setGraphData] = useState({ nodes: INITIAL_NODES, links: INITIAL_LINKS });
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      setGraphData({ nodes: INITIAL_NODES, links: INITIAL_LINKS });
      return;
    }

    const matchedNode = INITIAL_NODES.find((node) =>
  ...
  const handleNodeClick = (node: any) => {
    ...
    const fullNode = INITIAL_NODES.find(n => n.id === node.id);
    if (fullNode) {
